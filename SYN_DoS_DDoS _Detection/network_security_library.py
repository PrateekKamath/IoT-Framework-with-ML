...
class SYN_DoS_DDoS:
    def __init__(self, iface_name='Wi-Fi', allowed_IP=None, capture_limit=500):
        ...
        self.broker_address = "test.mosquitto.org"  # Example Mosquitto broker
        self.port = 1883  # Default port for Mosquitto
        self.topic = "packet_capture_results"
    ...
    def publish_results_via_mqtt(self, message):
            # MQTT client setup
    ...
    def prediction(self):
        ...
        if count_2 > count_1 and self.syn_flag_counts.get(majority_syn_flag, 0) > len(predictions) / 2 and self.source_ip_counts.get(majority_ip, 0) > len(predictions) / 2:
            output_message = f"SYN DoS attack detected from IP Address: {majority_ip} at {formatted_time}"
        elif self.syn_flag_counts.get(1, 0) > len(predictions) / 2 and len(self.source_ip_counts) > 1:
            output_message = f"SYN DDoS Attack Detected from multiple addresses at {formatted_time}"
        else:
            output_message = f"Normal Traffic: {formatted_time}"

        # Publish the output message to MQTT topic
        self.publish_results_via_mqtt(output_message)
    ...
    def print_devices(self):
        connected_devices_topic = 'connected_devices'  # Assuming 'connected_devices' is the topic for device info
        for device in self.devices:
            device_info = f"IP: {device['ip']}, MAC: {device['mac']}"
            self.publish_results_via_mqtt(device_info)  # Change made here: Passing only device_info
